...
def select_action(state, current_reward=0):
    global steps_done, eps
    sample = random.random()
    eps_threshold = decay_epsilon(steps_done, current_reward=current_reward, eps=eps, MODE='RBED',start_val=EPS_START, end_val=EPS_END, target_reward=500, target_increment=1)
    eps = eps_threshold
    eps_arr.append(eps)
    if steps_done % 100 == 0:
        print(eps_threshold)
    ...
    for i_episode in range(num_episodes):
        ...
        for t in count():
            if i_episode == 0:
                ...
                action = select_action(state, episode_durations[-1])
