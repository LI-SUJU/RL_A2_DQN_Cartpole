...
def select_action_with_decay(state):
    global steps_done
    sample = random.random()
...
def training_with_decay(num_episodes):
    ...
    for i_episode in range(num_episodes):
        ...
        state = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
        for t in count():
            action = select_action_with_decay(state)
            observation, reward, terminated, truncated, _ = env.step(action.item())
            reward = torch.tensor([reward], device=device)
            done = terminated or truncated
...
episode_durations = training_with_decay(num_episodes)
plt.plot(episode_durations, alpha=0.1, color="orange")
# please plot a line using smooth() function from plotHelper.py
plt.plot(smooth(episode_durations, 30), label=f'epsilon: decay from 0.9', alpha=1.0, color="orange")
...
plt.plot(smooth(episode_durations, 30), label=f'epsilon: {EPS_START}', alpha=1.0, color="firebrick")
########################################################################################
########################################################################################
steps_done = 0
EPS_START = 0.5
...
plt.plot(smooth(episode_durations, 30), label=f'epsilon: {EPS_START}', alpha=1.0, color="navy")
########################################################################################
steps_done = 0
EPS_START = 0.3
policy_net = DQN(n_observations, n_actions).to(device)
...
plt.plot(smooth(episode_durations, 30), label=f'epsilon: {EPS_START}', alpha=1.0, color="olive")
########################################################################################
...
plt.xlabel('Episode')
plt.ylabel('Rewards')
plt.title('Comparisons of DQN Training with Different Epsilon')
plt.legend()
# Add text
...
import os
os.makedirs('./plots/epsilon_tune', exist_ok=True)
# Save plot
plt.savefig(f'./plots/epsilon_tune/DQN_{num_episodes}_epsilon_tune.png')
